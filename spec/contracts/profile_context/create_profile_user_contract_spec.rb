...
describe ProfileContext::CreateProfileUserContract, type: :contract do
  describe '#call' do
    ...
    context 'when email attributes has invalid format' do
    ...
    context 'when password is not equal to password_confirmation' do
      let(:user) { create(:user) }
      let(:attributes) { attributes_for(:user, email: user.email, password_confirmation: 'another') }
